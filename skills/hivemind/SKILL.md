...
3. Copy the bundled assets into place:
   - `commands/*.md` -> `~/.claude/commands/` (the `/hive`, `/oc`, `/swarm`, ... entry points)
...
Slash commands (registered by the plugin; manual installs copy `commands/*.md` into `~/.claude/commands/`):
